class Sudoku {
  constructor() {
    ...
    this.MAX_COUNT = 81; // Number of squares
  ...
  _cross(ROWS, COLS) {
    ...
    // var COLS = this.DIGITS; // Column lables
  ...
  generate(
    ...
    for (si in shuffled_squares) {
      ...
      if (
        ...
        if (this.solve(board).error === false) {
          return board;
        }
        return "";
      }
  ...
  solve(board, reverse = false) {
    ...
    //   return {
    //     error: true,
    //     errorMessage: "Too few givens. Minimum givens is " + this.MIN_GIVENS,
    //     solution: "",
    //   };
    // }
  ...
  _eliminate(candidates, square, val) {
    ...
    for (let ui in this.SQUARE_UNITS_MAP[square]) {
      ...
      if (val_places.length === 0) {
        ...
        candidates_new = this._assign(candidates, val_places[0], val);
  ...
  validate_board(board) {
    ...
    if (board.length !== this.MAX_COUNT) {
      return "Invalid board size. Board must be exactly " + this.MAX_COUNT +
        " squares.";
    }
    ...
    for (var i in board) {
      if (
        !this._in(board[i], this.DIGITS) && board[i] !== this.BLANK_CHAR
      ) {
...
function handleInputChange(e) {
  ...
  let newBoard = textArea.value;
  if (sudoku.DIGITS.includes(val) || val === ".") {
    const position = sudoku.SQUARES.indexOf(id);
    if (position === 0) {
      newBoard = val + newBoard.slice(1, newBoard.length);
    } else if (position === 80) {
      newBoard = newBoard.slice(0, newBoard.length - 1) + val;
    } else {
      newBoard = newBoard.slice(0, position) + val +
        newBoard.slice(position + 1, newBoard.length);
    }
    if (newBoard.length === 81) {
      setTextArea(newBoard);
    }
  } else {
    e.target.value = "";
  }
